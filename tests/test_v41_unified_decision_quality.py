...
def test_v41_release_identity():
 ...
 assert json.loads((ROOT/'app/release.json').read_text(encoding='utf-8'))['release_name']=='Autonomous Diagnostics & Regime Intelligence'
...
def test_canonical_visible_versions_are_current():
 ...
 for row in p['canonical_pages']:
  ...
  assert all(v=='44.0.0' for v in visible),(row['path'],visible)

...
def test_release_identity():
 assert (ROOT/'VERSION').read_text(encoding='utf-8').strip()=='37.0.0'
...
def test_account_intelligence_output_exists_and_is_advisory():
 ...
 assert data['application_version']=='37.0.0' and data['read_only'] is True
 assert data['status'] in {'HEALTHY','PARTIAL','UNAVAILABLE'}
def test_threecommas_workflow_rebuilds_downstream_state():
 text=(ROOT/'.github/workflows/threecommas-update.yml').read_text(encoding='utf-8')

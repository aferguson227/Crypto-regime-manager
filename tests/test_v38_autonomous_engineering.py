...
def test_v38_identity_and_routes():
 ...
 assert release['release_name']=='Autonomous Market Research & Independent Trade Accounting'
...
def test_engineering_json_outputs_present():
 for n in ['github_actions_health.json','engineering_health.json','release_readiness.json','decision_quality.json']:
  p=ROOT/'docs'/n
  assert p.exists(),n
  assert json.loads(p.read_text(encoding='utf-8'))['application_version']=='46.0.0'

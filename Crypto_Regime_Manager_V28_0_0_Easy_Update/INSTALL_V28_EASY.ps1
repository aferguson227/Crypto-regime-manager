...
if (-not (Test-Path (Join-Path $ProjectRoot 'config.json'))) {
    throw "Place this extracted V28 update folder directly inside the Crypto Regime Manager project folder."
}
if (-not (Test-Path $UpdateRoot)) { throw "The update_files folder is missing. Extract the complete ZIP first." }

Write-Host "Installing Crypto Regime Manager V28.0.0..." -ForegroundColor Cyan
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$backup=Join-Path $ProjectRoot "backups\v28_0_$stamp"
...
Write-Host "V28.0.0 installed successfully." -ForegroundColor Green
Write-Host "Backup: $backup"
Write-Host "Next: commit and push, then run the V28 Autonomous Crypto Regime Refresh workflow once."

...
if (-not (Test-Path (Join-Path $ProjectRoot 'config.json'))) {
    throw "Place this extracted V26 update folder directly inside the Crypto Regime Manager project folder."
}
if (-not (Test-Path $UpdateRoot)) { throw "The update_files folder is missing. Extract the complete ZIP first." }

Write-Host "Installing Crypto Regime Manager V26.0.0..." -ForegroundColor Cyan
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$backup=Join-Path $ProjectRoot "backups\v26_0_$stamp"
...
Write-Host "V26.0.0 installed successfully." -ForegroundColor Green
Write-Host "Backup: $backup"
Write-Host "Next: commit and push, then run the V26 Autonomous Crypto Regime Refresh workflow once."

...
def fetch_json(url: str, retries: int = 3) -> dict[str, Any]:
    ...
    for attempt in range(retries):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "Crypto-Regime-Manager/24.1"})
...
def run(offline: bool = False) -> dict[str, Any]:
    ...
    result = {
        "version": "24.1.0", "generated_at": generated, "mode": "research_only", "source": source,
        ...
        "summary": {"symbols_reviewed": len(candidates), "eligible_symbols": len(eligible),
                    ...
                    "scan_duration_seconds": round(time.monotonic() - started_monotonic, 1),
    ...
    registry_path.write_text(json.dumps({"version":"24.1.0","generated_at":generated,"candidates":candidates}, indent=2), encoding="utf-8")
    return result

...
if (-not (Test-Path (Join-Path $ProjectRoot 'config.json'))) {
    throw "Place this extracted V24 update folder directly inside the Crypto Regime Manager project folder."
}
if (-not (Test-Path $UpdateRoot)) { throw "The update_files folder is missing. Extract the complete ZIP first." }

Write-Host "Installing Crypto Regime Manager V24.1.0..." -ForegroundColor Cyan
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$backup=Join-Path $ProjectRoot "backups\v24_1_$stamp"
...
python -m py_compile (Join-Path $ProjectRoot 'scripts\core\engine.py') (Join-Path $ProjectRoot 'scripts\core\research_queue.py') (Join-Path $ProjectRoot 'scripts\core\coin_discovery.py')
python -m pytest (Join-Path $ProjectRoot 'tests') -q
Write-Host "V24.1.0 installed successfully." -ForegroundColor Green
Write-Host "Backup: $backup"
Write-Host "Coin Discovery is research-only. Run: python scripts\coin_discovery.py"

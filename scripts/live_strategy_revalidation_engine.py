...
def main():
 ...
 for d in live.get('deals') or []:
  ...
  rows.append({'asset':a,'bot_name':d.get('bot_name'),'deal_open':True,'current_regime':w.get('current_regime_family'),
   ...
   'would_deploy_today':bool(o.get('optimisation_status')=='COMPLETE' and o.get('unseen_validation_pass')),
   'production_settings_comparison':'UNCHANGED_OR_CURRENT' if not o.get('winning_strategy_settings') else 'COMPARE_AFTER_DEAL_CLOSE',
   ...
   'next_deal_policy':'After this deal closes, compare production settings with the latest unseen-validated settings and current regime before the next deployment. Do not change the active deal mid-trade.',

...
def main()->int:
    now=datetime.now(timezone.utc).isoformat(); tc=read('threecommas.json'); kucoin=read('kucoin_account.json'); cloud=read('cloud_reliability.json'); diag=read('diagnostics_runtime.json') or read('diagnostics.json'); ws=read('professional_workspace.json'); cap=read('capital_intelligence.json'); cmd=read('command_state.json')
    active=[]
    endpoints=[]
    for name,row in (tc.get('endpoint_diagnostics') or {}).items():
        ...
        if row.get('status')!='pass':
            optional_balance=name.endswith('_balances') and row.get('category') in {'rate_limited','rate_limited_cached'}
            if optional_balance and str(kucoin.get('status') or '').lower()=='ok':
                continue
    ...
    data_score=100
    if tc.get('core_status') not in {None,'ok'}: data_score-=20
    ...
    payload={'schema_version':'1.0','application_version':application_version(),'generated_at':now,'overall':{'state':overall,'score_pct':overall_score,'active_issue_count':len(active),'critical_count':sum(i['severity']=='critical' for i in active),'warning_count':sum(i['severity']=='warning' for i in active)},'scores':scores,'threecommas':{'status':tc.get('core_status') or tc.get('status','unknown'),'last_attempt_at':tc.get('last_attempt_at'),'last_success_at':tc.get('last_success_at'),'last_success_age_hours':tc_age,'endpoints':endpoints,'accounts':len(tc.get('accounts') or []),'bots':sum(len(x.get('bots') or []) for x in (tc.get('assets') or {}).values()),'deals':len(deals)},'application':{'cloud_state':app_pipe.get('state','unknown'),'last_success_at':app_pipe.get('last_success_at'),'last_success_age_hours':app_age,'diagnostics_score_pct':system_score},'trading':{'active_deals':deals,'deal_count':len(deals)},'decision':{'primary_action':(ws.get('daily_decision') or {}).get('action'),'bot_name':(ws.get('daily_decision') or {}).get('bot_name'),'readiness_score_pct':decision,'ready_for_manual_action':(ws.get('decision_readiness') or {}).get('ready_for_manual_action',False)},'capital_source':cap.get('capital_source'),'kucoin_direct_status':kucoin.get('status'),'issues':active,'safeguards':{'read_only':True,'manual_approval_required':True}}

...
def run_walk_forward(root:Path,training_dir:Path,validation_dir:Path)->dict[str,Any]:
 ...
 with tempfile.TemporaryDirectory(prefix='crm_walk_forward_') as td:
  workspace=Path(td)
  train_groups={}
  for p in csv_sources(training_dir,workspace/'training'): train_groups.setdefault(symbol_key(p.name),[]).append(p)
  val_groups={}
  for p in csv_sources(validation_dir,workspace/'validation'): val_groups.setdefault(symbol_key(p.name),[]).append(p)
  train_files={k:choose_best_source(v) for k,v in train_groups.items()}
  val_files={k:choose_best_source(v) for k,v in val_groups.items()}

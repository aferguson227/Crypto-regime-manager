...
def main():
 tc=load('threecommas.json');trade=load('trade_intelligence.json');recon=load('execution_reconciliation.json');acct=load('independent_trade_accounting.json');cap=load('capital_intelligence.json')
 rby={(str(x.get('asset')).upper(),x.get('bot_id')):x for x in recon.get('deals') or []};tby={(str(x.get('asset')).upper(),x.get('bot_id')):x for x in trade.get('trades') or []}
 deals=[]
 for a,b in (tc.get('assets') or {}).items():
  for d in b.get('deals') or []:
   ...
   deals.append({'asset':str(a).upper(),'bot_id':d.get('bot_id'),'bot_name':d.get('bot_name'),'provider_status':d.get('status'),
    'effective_position_state':effective,'provider_stale':provider_stale,'opened_at':d.get('created_at'),'open_pnl_quote':t.get('profit_quote'),
    'profit_pct':t.get('profit_pct'),'capital_used_quote':d.get('capital_used_quote'),'distance_to_take_profit_pct':t.get('distance_to_take_profit_pct'),

...
def main():
 ...
 for x in wf.get('assets') or []:
  ...
  gates=[
   ...
   {'id':'dca_optimisation','label':'DCA settings optimisation','state':'PASS' if do.get('optimisation_status')=='COMPLETE' else 'PENDING',
    'detail':('Strategy settings are optimised on training data and frozen through unseen KuCoin validation.' if do.get('optimisation_status')=='COMPLETE'

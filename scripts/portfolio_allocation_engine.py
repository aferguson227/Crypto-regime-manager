...
ROOT=Path(__file__).resolve().parents[1];DOCS=ROOT/'docs';OUT=DOCS/'portfolio_allocation_recommendations.json';POL=ROOT/'config'/'v50_decision_policy.json'
def load(p):
...
def main():
 cap=load(DOCS/'capital_intelligence.json');bots=load(DOCS/'recommended_bots.json');p=load(POL);policy=p.get('allocation') or {}
 ...
 for x in ready:
  if usable is None:amount=None;reason='Deployable KuCoin capital is not currently available.'
   ...
   pct=min(float(policy.get('pilot_pct_of_deployable',18)),float(policy.get('max_single_bot_pct_of_deployable',25)))
   amount=round(max(float(policy.get('minimum_pilot_usdt',100)),usable*pct/100),2)
 ...
 payload={'schema_version':'1.0','application_version':application_version(),'generated_at':datetime.now(timezone.utc).isoformat(),'deployable_capital_usdt':deploy,'usable_after_cash_buffer_usdt':round(usable,2) if usable is not None else None,'recommendations':rows,'automatic_deployment':False,'capital_source':cap.get('capital_source'),'allocation_status':'KNOWN' if deploy is not None else 'WAITING_FOR_CAPITAL_TRUTH','next_phase':'Diversification/correlation weighting remains advisory groundwork; V50 never deploys automatically.'}

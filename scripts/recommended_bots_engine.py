...
def build():
    ...
    for c in pipe.get('candidates') or []:
        ...
        candidates.append({'candidate_id':cid,'asset':asset,'pair':c.get('pair') or f'{asset}-USDT','state':'READY_FOR_MANUAL_REVIEW',
          'confidence_pct':confidence,'suggested_pilot_usdt':suggested,'historical_max_capital':hist_max,
          ...
          'promotion_note':'Adding here saves a recommended-bot package only. It does not create, edit, start or stop a 3Commas bot.'})

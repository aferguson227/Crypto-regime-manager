...
def main():
 ...
 p={'schema_version':'1.0','application_version':(ROOT/'VERSION').read_text().strip(),'result':result,'issue_count':len(issues),'issues':issues,'checks':['no known raw enum leakage','mobile responsive rules','activity visibility','status formatting']}

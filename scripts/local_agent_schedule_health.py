...
def main():
 ...
 rows=[local,live,research];status='HEALTHY' if local['status']=='HEALTHY' and live['status']=='HEALTHY' else ('MISSING' if 'MISSING' in {local['status'],live['status']} else 'ATTENTION')
 payload={'schema_version':'3.1','application_version':application_version(),'generated_at':datetime.now(timezone.utc).isoformat(),'status':status,'task_name':local['task_name'],'exists':local['exists'],'enabled':local['enabled'],'last_run':local['last_run'],'next_run':local['next_run'],'last_result':local['last_result'],'tasks':rows,'live_data_service_status':live['status'],'live_data_service_mechanism':'HKCU per-user startup','research_worker_status':research['status']}
 OUT.write_text(json.dumps(payload,indent=2),encoding='utf-8');print(f'Background service health: local={local["status"]} live={live["status"]} research={research["status"]}');return 0

#!/usr/bin/env python3
"""V64 Cross-Exchange Continuation Resolver 2.0.\n\nLegacy states recognised for migration only: KRAKEN_EVIDENCE_MATERIALISING, KUCOIN_HISTORY_ACQUIRING, KUCOIN_HISTORY_DOES_NOT_REACH_CUTOFF, KUCOIN_CONTINUATION_GAP.
...
def main():
 ...
 for coin in reg.get('coins') or []:
  ...
  rec={'asset':asset,'resolver_version':'2.0','diagnostic_stage':'archive discovery','kraken_cutoff_status':coin.get('status'),'kraken_cutoff_mark_to_market_pnl':q.get('mark_to_market_pnl'),'original_validation_preserved':True,
  ...
  if not settings:
   rec.update({'continuation_status':'UNAVAILABLE_FROZEN_SETTINGS','terminal':True,'diagnostic_stage':'position reconstruction','reason':'The original Kraken registry does not contain the frozen DCA settings required to reconstruct the position.'});rows.append(rec);continue
  ...
  if not state:
   rec.update({'continuation_status':'RECONSTRUCTION_MISMATCH','terminal':True,'diagnostic_stage':'position reconstruction','reason':'Current deterministic replay no longer reconstructs the historical open state; original registry remains authoritative and this discrepancy requires research review.'});rows.append(rec);continue
  ...
  rec.update({'continuation_status':result.get('status'),'terminal':True,'diagnostic_stage':'continuation replay','reconstructed_open_state':state,'continuation':result,
 ...
 payload={'schema_version':'1.0','application_version':application_version(),'generated_at':now(),'method':'Reconstruct frozen Kraken cutoff open position, then continue it using later KuCoin 4h candles only.',
  ...
  'awaiting_evidence':sum(not x.get('terminal',False) for x in rows),'terminal_results':sum(bool(x.get('terminal')) for x in rows)}, 'safeguards':{'original_validation_rewritten':False,'research_only':True}}

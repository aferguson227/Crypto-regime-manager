...
def main():
 rb=load('regime_backtest_intelligence.json');oq=load('optimisation_queue.json');rp=load('research_pipeline.json');items=oq.get('items') or []
 states={}
 ...
 payload={'schema_version':'1.0','application_version':application_version(),'generated_at':datetime.now(timezone.utc).isoformat(),'status':'ACTIVE','automatic':True,'research_cycle_hours':24,
  'summary':{'candidates_under_review':len(items),'regime_research_complete':states.get('REGIME_RESEARCH_COMPLETE',0),'optimisation_pending':states.get('INDEPENDENT_BASELINE_PASS_REGIME_RESEARCH_PENDING',0),'rejected':states.get('REJECTED',0),'backtests_last_cycle':(rb.get('summary') or {}).get('backtests_run',0),'datasets_last_cycle':(rb.get('summary') or {}).get('datasets',0)},
  ...
  'process':['Discover candidates','Classify regime','Test entry triggers','Optimise DCA settings','Penalise drawdown and capital lock','Freeze training winner','Validate independently','Confirm current KuCoin/global regime','Surface manual deployment candidate'],

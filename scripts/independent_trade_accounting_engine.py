...
def main():
 ...
 legacy_limited=str(ledger.get('realised_profit_status') or '')=='HISTORICAL_COST_BASIS_UNAVAILABLE'
 realised=(ledger.get('realised_profit_quote') if ledger.get('realised_profit_quote') is not None else
           (ledger.get('partial_matched_realised_profit_quote') if legacy_limited else tc.get('realised_profit_usdt')))
 baseline={'active':legacy_limited,
  'label':'CRM accounting baseline established' if legacy_limited else None,
  'historical_exception_count':int(((ledger.get('accounting_limitation') or {}).get('unmatched_sell_count') or 0)),
  'principle':'After the exhausted 730-day search, the unresolved pre-baseline sale is excluded rather than estimated. All provably matched and future KuCoin fills continue in the independent ledger.' if legacy_limited else None}
 p={'schema_version':'1.0','application_version':application_version(),'generated_at':datetime.now(timezone.utc).isoformat(),'capital_source':'local KuCoin private read-only account snapshot','execution_telemetry_source':'3Commas until direct KuCoin fill ledger is enabled','active_trade_count':len(trades),'deployed_quote':round(deployed,4),'open_pnl_quote':round(open_pnl,4) if open_pnl is not None else None,'open_pnl_status':'KNOWN' if open_pnl is not None else 'WAITING_FOR_CURRENT_PRICE_OR_ENTRY','realised_profit_quote':realised,'realised_profit_status':('KNOWN_WITH_HISTORICAL_EXCEPTION' if legacy_limited else ('KNOWN' if realised is not None else str(ledger.get('realised_profit_status') or 'BUILDING_KUCOIN_FILL_COST_BASIS'))),'accounting_baseline':baseline,'realised_profit_source':('KuCoin persistent fill ledger from established accounting baseline' if legacy_limited else ('KuCoin persistent fill ledger' if ledger.get('realised_profit_quote') is not None else ('3Commas fallback' if tc.get('realised_profit_usdt') is not None else 'KuCoin persistent fill ledger building'))),'portfolio_value_quote':ku.get('total_usdt_equivalent') or ku.get('total_equity_usdt'),'read_only':True,'execution_enabled':False,'reconciliation_status':'PARTIAL' if trades else 'CAPITAL_ONLY','migration_note':'V50 persists recent KuCoin spot fills locally so closed-fill cost basis grows across refreshes and upgrades. Realised P/L is published only when the ledger can match the relevant sell fills to known cost basis.'}

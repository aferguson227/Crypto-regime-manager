...
if (-not (Test-Path (Join-Path $ProjectRoot 'config.json'))) {
    throw "Place this extracted V27 update folder directly inside the Crypto Regime Manager project folder."
}
if (-not (Test-Path $UpdateRoot)) { throw "The update_files folder is missing. Extract the complete ZIP first." }

Write-Host "Installing Crypto Regime Manager V27.0.0..." -ForegroundColor Cyan
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$backup=Join-Path $ProjectRoot "backups\v27_0_$stamp"

...
const lifecycleReady=lifecycleBots.filter(x=>['READY_TO_DEPLOY','RECOMMENDED_NOW','READY_FOR_DEPLOYMENT_REVIEW'].includes(x.lifecycle_state));
...
const totalCapital=port.total_equity??kucoin.usdt_balance??null;
const availableCapital=alloc.deployable_quote??port.deployable_capital??null;
const reservedCapital=port.reserved_capital??null;

...
function renderManagedBots(){
 ...
 const rows=all.filter(x=>x.state==='LIVE'||selected.has(String(x.asset||'').toUpperCase()));
 ...
 root.innerHTML=display.length?`<div class="crm-managed-table"><div class="crm-managed-head"><span>Bot</span><span>State</span><span>Position</span><span>SO</span><span>P/L</span><span>Reserve / need</span><span>Action</span></div>${display.map(x=>{
...
document.addEventListener('click',e=>{
 ...
 const add=e.target.closest('.crm-add-my-bot');if(add){const set=myBotAssets();set.add(String(add.dataset.asset||'').toUpperCase());saveMyBotAssets(set);renderManagedBots();add.textContent='In My Bots'}
 const rem=e.target.closest('.crm-remove-my-bot');if(rem){const set=myBotAssets();set.delete(String(rem.dataset.asset||'').toUpperCase());saveMyBotAssets(set);renderManagedBots();document.querySelectorAll(`.crm-add-my-bot[data-asset="${CSS.escape(rem.dataset.asset)}"]`).forEach(x=>x.textContent='Add to My Bots')}
});

...
if(brief){
 brief.innerHTML=`
  <div class="crm-trader-metrics">
   ...
   ${stat('Safe to allocate now',CRMFormat.quote(safeAllocate,'USDT'),safeAllocate===0?(portfolioCapitalV2.conditional_multi_bot_capacity_usdt>0?`Conservative safe pool is 0. Conditional capacity ${CRMFormat.quote(portfolioCapitalV2.conditional_multi_bot_capacity_usdt,'USDT')} exists under expected DCA depth, but remains advisory only.`:'All currently free capital is being protected for active DCA commitments.'):'Capital CRM considers available for another approved strategy.')}
   <div class="crm-stat"><span>Open P/L</span><strong id="crm-live-open-pnl" class="crm-fit-text">${esc(pnlValue(openPnl,openPnlPct))}</strong><small id="crm-live-open-pnl-note" class="crm-muted">${esc(openPnl==null?'Waiting for a current KuCoin price and provable position cost basis.':`KuCoin mark-to-market · priced ${ageText(liveTruth.open_pnl_priced_at)}${liveTruth.open_pnl_status==='LIVE'?'':' · fallback data'}`)}</small></div>
   ${stat('Realised P/L',realisedPnl!=null?pnlValue(realisedPnl,realisedPnlPct):realisedStatus,realisedPnl!=null?'Closed-trade P/L · percentage shown relative to current portfolio value.':realisedDetail)}
   <div class="crm-stat"><span>Trading P/L</span><strong id="crm-live-total-pnl" class="crm-fit-text">${esc(totalPnl!=null?pnlValue(totalPnl,totalPnlPct):partialTotalText)}</strong><small id="crm-live-total-pnl-note" class="crm-muted">${esc(totalPnl==null?'This is deliberately partial until historical realised P/L is complete.':'Open + realised P/L · percentage shown relative to current portfolio value.')}</small></div>
  </div>

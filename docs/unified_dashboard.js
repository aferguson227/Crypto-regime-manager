...
const safeAllocate=portfolioAllocation.deployable_capital_usdt??capital.safe_to_allocate_now??capital.deployable_capital??alloc.deployable_quote??port.deployable_capital??null;
...
if(brief){
 brief.innerHTML=`
  <div class="crm-trader-metrics">
   ...
   ${stat('Open P/L',openPnl==null?'Updating from KuCoin live price':CRMFormat.quote(openPnl,'USDT'),openPnl==null?'Waiting for a current KuCoin price and provable position cost basis.':`KuCoin mark-to-market · priced ${ageText(liveTruth.open_pnl_priced_at)}${liveTruth.open_pnl_status==='LIVE'?'':' · fallback data'}`)}

...
if (-not (Test-Path (Join-Path $ProjectRoot 'config.json'))) {
    throw "Place this extracted V29 update folder directly inside the Crypto Regime Manager project folder."
}
if (-not (Test-Path $UpdateRoot)) { throw "The update_files folder is missing. Extract the complete ZIP first." }

Write-Host "Installing Crypto Regime Manager V29.0.0..." -ForegroundColor Cyan
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$backup=Join-Path $ProjectRoot "backups\v29_0_$stamp"
...
python -m py_compile (Join-Path $ProjectRoot 'scripts\core\engine.py') (Join-Path $ProjectRoot 'scripts\core\research_queue.py') (Join-Path $ProjectRoot 'scripts\core\coin_discovery.py') (Join-Path $ProjectRoot 'scripts\core\candidate_validation.py') (Join-Path $ProjectRoot 'scripts\candidate_validation.py') (Join-Path $ProjectRoot 'scripts\cloud_update.py') (Join-Path $ProjectRoot 'scripts\core\data_import.py') (Join-Path $ProjectRoot 'scripts\core\backtest_lab.py')
python -m pytest (Join-Path $ProjectRoot 'tests') -q
Write-Host "V29.0.0 installed successfully." -ForegroundColor Green
Write-Host "Backup: $backup"
Write-Host "Next: commit and push, then create a data\imports folder for Kraken CSVs, then run the V29 Autonomous Crypto Regime Refresh workflow once."

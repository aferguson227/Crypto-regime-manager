...
def build(root: Path) -> dict[str, Any]:
    docs=root/'docs'; strategies=_read(docs/'strategies.json',{}); discovery=_read(docs/'coin_discovery.json',{}); validation=_read(docs/'candidate_validation.json',{}); queue=_read(docs/'research_queue.json',{}); lab=_read(docs/'backtest_lab.json',{}); imports=_read(docs/'import_registry.json',{})
    ...
    payload={
      'version':'29.0.0','generated_at':datetime.now(timezone.utc).isoformat(),'mode':'unified_advisory_decision_engine',
      'best_setup':best,'production_ranking':production,'all_assets':ranked,
      'pipeline':{'production_assets':len(production),'eligible_production':len(eligible),'discovery_candidates':len(discovered),'validation_candidates':len(validation_candidates),'research_ready':len(research_ready),'queue_items':len(queue.get('items') or queue.get('queue') or []),'imported_datasets':imports.get('ready',0),'backtested_datasets':len(lab.get('datasets') or [])},
      ...
      'strategy_factory':{'lifecycle':['DISCOVERED','BACKTESTING','OPTIMISING','FORWARD VALIDATION','PAPER TRADING','PRODUCTION','MONITORING','RETIRED'],'imports':imports,'backtest_registry':lab},'policy':{'discovery_can_change_live_ranking':False,'validation_required_before_production':True,'manual_approval_required':True,'automatic_live_changes':False},
